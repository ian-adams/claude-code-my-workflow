...
::: {.callout-tip}
## Case Study: A PhD Course

This workflow was developed over 6+ sessions building a PhD-level course. The result: 6 complete lectures (140+ slides each), with Beamer + Quarto versions, interactive Plotly charts, TikZ diagrams, and R replication scripts --- all managed by the orchestrator and reviewed by 10 specialized agents across 5 quality dimensions.
:::
...
In practice, verification caught unverified TikZ diagrams that would have deployed with overlapping labels, broken SVGs in Quarto slides that wouldn't display, and R scripts with missing intercept terms that produced silently wrong estimates.
:::
...
If you forked this template (rather than cloning fresh), you can pull improvements from the upstream template as they are released.

### Setup

Point `upstream` at the template repo you forked from:
...
The `/sync-upstream` skill handles this. In check mode (the default), it fetches upstream, lists new commits, and categorizes changes into **infrastructure** (skills, agents, rules, scripts --- safe to merge) and **project content** (CLAUDE.md, slides, data --- likely to conflict in customized projects).

```
/sync-upstream
```

A session-startup rule also checks automatically: if upstream has new commits, Claude will mention it when you start a session.
...
This creates a `sync-upstream-YYYY-MM-DD` branch, merges upstream changes, and opens a PR against your repo. If there are merge conflicts, the skill stops and advises on resolution --- infrastructure conflicts can usually accept the upstream version, while project content conflicts need manual resolution.

::: {.callout-tip}
## Infrastructure vs. Project Content

Infrastructure files (`.claude/skills/`, `.claude/agents/`, `.claude/rules/`, `scripts/`) are designed to merge cleanly because they add new capabilities without touching your customizations. Project content (`CLAUDE.md`, `Slides/`, `Quarto/`) will almost always conflict in a customized project --- and that is expected. Resolve these manually.
:::
...
| `/sync-upstream` | Check/merge upstream template changes | After template repo updates |
...
The template includes four Quarto slide themes: `emory-clean.scss`, `usc-clean.scss`, `uno-clean.scss`, and `academic-clean.scss` (a generic Charcoal/Teal theme). To customize, pick one as your starting point and:
...
3. **Use the [LEARN] format.** Every correction was tagged and persisted. This prevents repeating mistakes.
4. **Trust the adversarial pattern.** The critic-fixer loop catches things you won't. Let it run.
5. **Verify everything.** The verification rule exists for a reason. Never skip verification.
...
| `/commit` | `.claude/skills/commit/` | Stage, commit, PR, merge cycle |
| `/sync-upstream` | `.claude/skills/sync-upstream/` | Check/merge upstream template changes |
...
| Upstream Sync Reminder | `.claude/rules/upstream-sync-reminder.md` | Check for upstream changes at session startup |
